...
struct ClothMeshData
{
  ...
  std::shared_ptr<SimpleMesh>   pNorm; // 
  ...
  void initGPUDataTr(GLuint a_programId)    { pTris->CreateGPUData(a_programId); }
  void initGPUDataNorm(GLuint a_programId)  { pNorm->CreateGPUData(a_programId); }
